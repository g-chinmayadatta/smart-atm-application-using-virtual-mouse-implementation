 ...
 $servername = "localhost";
    ...
    if ($result->num_rows > 0){
        /*if($result2->num_rows>0){
            echo "account no: *********** ".substr($row4['ac_no'],-4);
            echo "transaction id is ". $utra_id=$row['TRANSACTION_ID'];
            echo "transactin amount ".$uamount=$row['AMOUNT'];
            echo "time transaction is ".$utime=$row['tmp_time'];
            echo  "avilable amount in account ".$avil=$row2['CARD_BALANCE'];
            echo '<button onclick="window.location.href=\'http://127.0.0.1/project/index.html\'">exit</button>';
        }*/
        $utra_id=$row['TRANSACTION_ID'];
...
<html>
    ...
    <head>
        <body>
            <section>
                ...
                <tr>
                    <td>Transfer to </td>
                    <td><?php echo $_SESSION["rename"];?></td>
                </tr>
                <tr>
  ...
  <!--<h2>UCEN Bank</h2><br>
  <h2><?php echo "Account No : *********** ".substr($row4['ac_no'],-4)?></h2><br>
  <h2> TRANSFER TO : <?php echo $_SESSION["rename"];?></h2><br>
  <h2>Transaction Id : <?php  echo $utra_id=$row['TRANSACTION_ID'];?></h2><br>
  <h2>Transaction type : <?php echo $utra_type=$row5["Transaction_type"];?></h2><br>
  <h2>Transaction status : <?php echo $utra_status=$row5["Transaction_status"];?></h2><br>
  <h2>Transaction Amount :  <?php echo $uamount=$row['AMOUNT'];?></h2><br>
  <h2>Time of Transaction : <?php echo $utime=$row['tmp_time'];?></h2><br>
  <h2>Availabe Amount : <?php echo $avil=$row2['CARD_BALANCE'];?></h2><br><br>
  <button name="download" class="button"  type="submit" onclick="exportTableToExcel('receipt', 'receipt')">download</button><br><br>
  <button type="submit" class="button" onclick="window.location.href='index.html'">Exit</button>
  <br><br><br>-->
</div>
